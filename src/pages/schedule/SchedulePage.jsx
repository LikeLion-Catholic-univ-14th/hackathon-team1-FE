...
function SchedulePage() {
  ...
  useEffect(() => {
    // 나중에 fetchDailyDetail(selectedDate).then(setDaily).catch(() => setDaily(null))
      ...
      <div className="relative">
            ...
            {cards.length > 0 ? (
              cards.map((card) => (
                <DayUvCard
                  ...
                  date={selectedDate}
                  info={card}
                  canToggle={daily?.scheduleId != null}
                />
              ))
            ) : (
        ...
        {showUpload && (
          <ScheduleUploadModal
            ...
            onUpload={(file) => {
              console.log('업로드', file, hasSchedule ? '수정' : '신규')
              setShowUpload(false)
              // 공통화 완료되면 여기서 ScheduleConfirmModal 열기
            }}
          />
        )}
      </div>
